//
//  FeedbackReleaseGate.swift
//  Pantrix
//
//  The impure half of the feedback release gate (§4a): probes whether this is a development build, then
//  defers the decision to the Kit's `FeedbackAvailability.decide` (which is where the logic is tested). A
//  build is a dev context on the Simulator, or when it carries the `get-task-allow` entitlement — read from
//  the embedded provisioning profile. App Store and TestFlight builds have neither, so feedback stays dark
//  there unless the host turns off `debugOnly`.
//
//  Re-implemented here rather than reused from `PantrixInspector`: feedback is a SEPARATE opt-in product and
//  cannot depend on the inspector. iOS 15-gated to satisfy the view-target availability rule; it has no iOS 15
//  dependency of its own but is only ever reached from `FeedbackRuntime`.
//
...
struct FeedbackReleaseGate {
    ...
    func isAvailable(debugOnly: Bool) -> Bool {
        FeedbackAvailability.decide(
